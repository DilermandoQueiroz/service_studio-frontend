...
const Signup  = () => {
  ...
  const { user, signup, sendEmailValidation } = useAuth()
  ...
  const myOnSubmit: SubmitHandler<IFormInput> =  async (submit) => {
    ...
    try {
      const userCredential = await signup(submit.email, submit.password)
      await sendEmailValidation(userCredential.user)
      setData({
        ...data,
        email: submit.email,

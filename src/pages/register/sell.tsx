...
import { IPostSell } from '../../types/index'
import "react-datepicker/dist/react-datepicker.css";
import { Sell } from '../../service/Sell'



const RegisterSell = ({auth, pathname}) => {

  const router = useRouter();
  const { control, register, handleSubmit } = useForm<IPostSell>();
  const [loading, setLoading] = useState(false)
  const {user} = auth

  const myOnSubmit: any = async (submit: IPostSell) => {
    setLoading(true)
    try {
      console.log(submit)
      if(await Sell.createSell(submit, user.email)) {
...
export default withProtected(RegisterSell)